...
function controlsInitialization() {
    ...
    sendItemsToPopup({items: ffStorage.storage.appendedItems ? ffStorage.storage.appendedItems : []});
    ...
    sendUnreadItemsCount( ffStorage.storage.unreadItemsCount ? ffStorage.storage.unreadItemsCount : 0);
...
function reloadPanel() {
    ...
    getItems(prepareItemsAndSendItemsToPopup);
    appGlobal.panel.port.emit("backToTop");
}
...
function resetItemsReaded(){
    if( ffStorage.storage.appendedItems!=null){
        ...
        tabs.open({
            ...
            onReady: function(tab){
                ...
                worker.port.on("testNotifyBtn", function(){
                    var testItems;
                    if( ffStorage.storage.appendedItems!=null && ffStorage.storage.appendedItems.length>0){
...
function  _updateItems(response){
    if (response["error"] == 0) {
        ...
        if (data && data.length > 0) {
            ...
            //过滤下
            var appendItems =  ffStorage.storage.appendedItems!=null ? ffStorage.storage.appendedItems : []
...
function updateItems(callback) {
    ...
    apiRequestWrapper({
        parameters: {
            lasttime:  ffStorage.storage.lasttime != null ? ffStorage.storage.lasttime : "1",
            mod: "get_post_twenty"
        },
        onSuccess: function (response) {
...
function readOptions() {
    for (var optionName in appGlobal.options) {
        if(ffStorage.storage[optionName]!=null){
            console.info('appGlobal.options[',optionName,"]===========",ffStorage.storage[optionName]);
